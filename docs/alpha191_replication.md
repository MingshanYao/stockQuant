...
### 12.7 新版分级回撤控制框架验证 (2026-06-15)

基于 12.6 中的分析，我们设计并实现了完整的四级回撤控制框架，以替代原有的二元止损+熔断机制。

**框架架构（感知→决策→执行）：**

```
感知层
├── MarketRegimeDetector  市场状态识别 (牛/正常/熊/危机)
│   └── MA20/MA60 趋势 + 20d/252d 波动率比值 + 5日急跌检测
└── RiskMonitor           分级回撤预警 (绿/黄/橙/红)
    └── 绿(DD<4%, scale=1.0) → 黄(4-7%, scale=0.8) → 橙(7-10%, scale=0.6) → 红(≥10%, scale=0.3)

决策层
├── 市场状态 → 仓位上限: 牛=100% / 正常=80% / 熊=50% / 危机=20%
├── 回撤预警 → 仓位压缩: 最终仓位系数 = 市场系数 × 回撤系数
└── StopLossManager (trailing模式): 移动止损 + 固定止盈
```

**测试配置：** 5 组风险参数 × 2 个组合（All_ICIR加权 / All_等权），共 10 个回测。

| 配置 | enable | 移动止损 | 止盈 | 说明 |
|------|--------|---------|------|------|
| 无风控(基线) | False | 8% | 20% | 无任何风控，始终满仓 |
| 新框架_默认参数 | True | 5% | 20% | 框架内置阈值 + 中等个股止损 |
| 新框架_紧止损 | True | 3% | 15% | 框架内置阈值 + 较紧个股止损 |
| 新框架_松止损 | True | 8% | 25% | 框架内置阈值 + 较松个股止损 |
| 新框架_仅靠框架 | True | 50% | 50% | 仅靠市场+DD仓位控制，无个股止损 |

**回测结果（2010-2016，全A股，初始资金 100 万）：**

| 策略 | 年化收益 | 最大回撤 | 夏普比率 | 卡玛比率 | Alpha |
|------|---------|---------|---------|---------|-------|
| All_等权_无风控(基线) | +38.55% | -48.49% | 1.39 | 0.80 | +0.35 |
| All_ICIR加权_无风控(基线) | +22.54% | -49.70% | 0.76 | 0.45 | +0.19 |
| All_等权_仅靠框架 | +21.08% | -19.66% | **1.78** | **1.07** | +0.18 |
| All_等权_松止损 | +13.97% | -18.99% | 1.11 | 0.74 | +0.11 |
| All_ICIR加权_仅靠框架 | +9.86% | -20.99% | 0.76 | 0.47 | +0.07 |
| All_等权_紧止损 | +8.44% | -18.40% | 0.59 | 0.46 | +0.05 |
| All_ICIR加权_松止损 | +7.71% | -18.06% | 0.52 | 0.43 | +0.05 |
| All_等权_默认参数 | +7.70% | -19.12% | 0.51 | 0.40 | +0.05 |
| All_ICIR加权_默认参数 | +4.01% | -18.55% | 0.12 | 0.22 | +0.01 |
| All_ICIR加权_紧止损 | +3.71% | -18.16% | 0.08 | 0.20 | +0.01 |

**净值曲线对比：** `docs/image/alpha191/risk_management_equity.png`

**核心发现：**

1. **回撤大幅降低但未达标：** 新框架将最大回撤从 ~49% 降至 ~18-21%（降幅约 60%），但仍远超 5% 目标。2015 年股灾期间的系统性下跌难以通过个股层面的止损完全规避。

2. **"仅靠框架"配置表现最优：** All_等权_仅靠框架 获得最高夏普 (1.78) 和卡玛比率 (1.07)，年化收益 +21.08%。说明市场状态识别 + 分级回撤预警的仓位控制比过度频繁的个股止损更有效——过紧的个股止损会增加交易成本和错误止损。

3. **等权优于 ICIR 加权：** 在有风控的配置下，等权组合全面优于 ICIR 加权组合。ICIR 加权组合中，风控后的 Alpha 几乎被完全抹去（从 +0.19 降至 +0.01），说明紧止损对高权重持仓的伤害更大。

4. **止损参数传导链：** 紧止损 (3% 移动止损) → 更频繁触发 → 更多交易成本 → 更低净收益。松止损 (8%) 在回撤控制接近的情况下，收益显著优于紧止损。

5. **框架内置参数已足够：** 市场状态系数和分级 DD 预警发挥了主要的回撤控制作用。增加个股移动止损的边际贡献有限（18.06% vs 18.99% vs 19.66% 的回撤差异不大），但收益被显著压缩。

**与旧版对比：**

| 指标 | 旧版(无风控) | 旧版(5%熔断) | 新版(等权_仅靠框架) |
|------|------------|-------------|-------------------|
| 最大回撤 | -48.49% | -48.04% | **-19.66%** |
| 年化收益 | +38.55% | +38.33% | +21.08% |
| 夏普比率 | 1.39 | 1.38 | **1.78** |

旧版二元熔断几乎无效（触发即清仓，然后被动承受），新版分级框架通过动态仓位管理将回撤降低了 60%，同时夏普反而提升。

**待实验回撤控制策略 (TODO)：**

*优先级排序基于预期效果 × 实现成本。每条包含：核心思路、预期回撤降幅、实现难度。*
...
- [ ] **1. 股指期货对冲**
  - 思路：在 DD 进入橙/红预警时，按组合 Beta 做空等市值的 IF/IC 期货，剥离系统性风险
  - 预期回撤降幅：~8-10pp（从 19% 降至 ~10-12%）
  - 成本：期货保证金 + 展期成本，年化约 2-3%
  - 关键技术点：实时 Beta 估计、对冲比例计算、展期时机选择

- [ ] **2. 行业集中度约束**
  - 思路：单行业仓位上限 20-30%，防止 2015 年 TMT 式的行业集中踩踏
  - 预期回撤降幅：~3-5pp
  - 成本：几乎为零（仅在调仓时增加行业约束检查）
  - 关键技术点：行业分类映射（申万/GICS）、行业归属判断逻辑

- [ ] **3. 波动率目标 (Volatility Targeting)**
  - 思路：`position = target_vol / realized_vol`，高波动时自动降仓，比固定阈值更平滑
  - 预期回撤降幅：~2-4pp
  - 成本：无直接成本，但可能降低牛市中收益
  - 关键技术点：滚动窗口选择、目标波动率参数标定、波动率估计方法

### 中优先级 — 需要更多数据或改造

- [ ] **4. 时间止损**
  - 思路：持仓超过 N 天仍未盈利则强制平仓，过滤无效持仓
  - 预期回撤降幅：~1-3pp
  - 成本：可能过早卖出"慢牛"型股票
  - 关键技术点：最优 N 值回测、与移动止损的协同配合

- [ ] **5. 分批建仓/减仓**
  - 思路：触发预警后分 2-3 批执行，减少冲击成本和过度反应
  - 预期回撤降幅：~1-2pp（主要改善执行质量）
  - 成本：实现复杂度较高，需要跨 bar 状态管理
  - 关键技术点：分批执行调度器、未完成订单的持久化

- [ ] **6. 相关性止损 (Correlation Stop)**
  - 思路：当多只持仓同时下跌（相关性骤升），触发组合层面降仓
  - 预期回撤降幅：~2-3pp
  - 成本：需实时计算持仓相关性矩阵
  - 关键技术点：相关性突变检测阈值、与 DD 预警的优先级关系

### 低优先级 — 锦上添花

- [ ] **7. 日内监控**
  - 思路：基于分钟数据实时检查 DD，比日线更及时
  - 预期回撤降幅：~1-2pp（减少日内极端波动损失）
  - 限制：需要分钟级数据源，对跌停/停牌无效

- [ ] **8. 期权尾部保护**
  - 思路：买入深度虚值看跌期权作为尾部风险保险
  - 预期回撤降幅：极端行情下可大幅减少尾部损失
  - 限制：持续权利金支出，适合大资金，国内期权品种有限

- [ ] **9. 参数自适应**
  - 思路：市场状态阈值和 DD 预警阈值根据市场阶段（牛熊周期）自适应调整
  - 预期回撤降幅：~1-3pp
  - 关键技术点：市场阶段划分规则、参数平滑过渡机制

---

**建议实施路线：** 行业约束 → 波动率目标 → 股指期货对冲 → 相关性止损 → 其他。前三个改动小见效快，是性价比最高的方向。
...
| `MarketRegimeDetector` | 市场状态识别器（牛/熊/震荡/危机） |
| `RiskMonitor` | 分级回撤预警监控器（绿/黄/橙/红） |
| `StopLossManager` | 多模式个股止损管理器（固定/移动/时间止损） |
| `AlphaFactorStrategy` | 通用截面因子选股策略（集成完整回撤控制框架） |
