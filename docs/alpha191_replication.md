...
1. **引入对冲机制：** 在回撤进入橙/红色预警时，通过做空股指期货对冲 Beta 风险，进一步压缩极端行情下的回撤。
2. **行业集中度限制：** 防止组合过度集中于单一行业（如 2015 年的 TMT），行业层面设置仓位上限。
3. **参数自适应：** 市场状态阈值和 DD 预警阈值应根据不同市场阶段（如牛熊周期）自适应调整。
4. **日内监控：** 基于分钟数据实时更新 DD 预警，避免收盘后才能反应的滞后性。
